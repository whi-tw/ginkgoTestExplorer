...
export const ginkgoImportsNotFoundError =
  'error creating outline: file does not import "github.com/onsi/ginkgo" or "github.com/onsi/ginkgo/extensions/table"';
...
export async function callGinkgoOutline(
  ginkgoPath: string,
  ...
  return await new Promise<string>((resolve, reject) => {
    ...
    if (p.pid) {
      if (!p.stdin) {
        return reject(
          new Error(
            `unable to write to stdin of ginkgo process: pipe does not exist`
          )
        );
      }
